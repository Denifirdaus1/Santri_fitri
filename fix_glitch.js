...
const updatedFn = `function renderBatasWaktuRealtime(container, izin) {
    ...
    // Render the container only once to avoid CSS animation restarts
    container.innerHTML = \`
    ...
    const updateHTML = () => {
        ...
        // Just in case State crosses border during a single session without refresh
...
c = c.replace(/function renderBatasWaktuRealtime\(container, izin\) \{[\s\S]*?countdownInterval = setInterval\(updateHTML, 1000\);\n\}/, updatedFn);
