const fs = require('fs');
const filePath = 'c:/Projectsekolah/santri_fitri/santri/index.html';
...
// 2. Inject HTML <div id="activeStatusContainer"></div> before MAIN CONTENT
const htmlToInject = `
...
content = content.replace('<!-- MAIN CONTENT -->', htmlToInject);

// 3. Inject loadActiveStatus and renderBatasWaktuRealtime Functions
const jsToInject = `
...
content = content.replace('// ==================== SET MIN DATE ====================', jsToInject);

// 4. Also call loadActiveStatus() inside the INIT IIFE since that's why the syntax error happened originally.
const callToInject = 'loadRiwayat();\\n    loadActiveStatus();\\n})()';
content = content.replace(/loadRiwayat\(\)\n}\)\(\)/g, callToInject);

fs.writeFileSync(filePath, content);
console.log('injected all UI additions');
